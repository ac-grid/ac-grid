...
export const routeMeta: Record<string, RouteMeta> = {
  ...
  "/examples": {
    ...
    keywords:
      "AC Grid examples, data grid examples, ag-grid alternative examples",
    image: "/og-image.png",
  },
  "/rfc": {
    title: "Completed RFCs | AC Grid – acgrid.dev",
    description:
      "Presentation pages and demo-react validation entry points for every completed AC Grid RFC.",
    keywords: "AC Grid RFC, data grid validation, AC Grid examples",
    image: "/og-image.png",
  },
  "/docs": {
    ...
    keywords:
      "AC Grid documentation, AC Grid guide, AC Grid API, ag-grid alternative",
    image: "/og-image.png",
  },
  // 404 页面（通配符路由）
  "*": {
...
export function getRouteMeta(path: string): RouteMeta {
  ...
  if (path.startsWith("/rfc/")) {
    const baseMeta = routeMeta["/rfc"] || routeMeta["/"];
    const rfcId = path.slice(5).split("/")[0];
    return {
      ...baseMeta,
      title: `RFC-${rfcId} | Completed RFCs – AC Grid`,
    };
  }
